 /******************************************************************************
 *
 * File name: avr_registers_types.h
 *
 * Description: Defines types and macros for accessing AVR micro-controller registers.
 * Includes unions for 8-bit and 16-bit registers, allowing both byte and bit-level
 * access for GPIO, MCU control, timers, ADC, and other hardware features.
 *
 ********************************************************************************/
...
typedef union {
    uint8 byte;                 // 8-bit unsigned integer
    struct {
        uint8 PSR10: 1;         // Pre-scaler Reset 10
        uint8 PSR2: 1;          // Pre-scaler Reset 2
